  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: `1px solid ${C.border}`, background: "#f8faff" }}>
      <span style={{ fontWeight: 800, fontSize: "0.9rem", color: C.text }}>
        <i className={`fas ${icon} me-2`} style={{ color: C.blue2 }} />{title}
      </span>
...
export default function ResultPage() {
  ...
  const academicData = liveStudent ? db.academicRecords.find(r => r.studentId === liveStudent.id) : null;
  const EXAMS = academicData?.EXAMS || [];
  const NOTICES = academicData?.NOTICES || [];
  const SEM_ALL = academicData?.SEM_ALL || [];
  const SUBJECTS = useMemo(() => academicData?.SUBJECTS || [], [academicData]);
    ...
    <div style={{ background: C.bg, minHeight: "100vh", padding: "24px 16px", fontFamily: "'Segoe UI', sans-serif", color: C.text }}>
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 24, flexWrap: "wrap", position: "relative" }}>
            ...
            <div style={{ flex: 1, minWidth: 180 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                {[
                  { ic: "fa-id-card", t: `Roll: ${studentRoll}` },
                  { ic: "fa-graduation-cap", t: studentGrade },
                  { ic: "fa-layer-group", t: "Semester IV · 2025–26" },
                  { ic: "fa-university", t: "Sri Venkateswara College" },
                ].map((m, i) => (
                  <span key={i} style={{ color: "rgba(255,255,255,.7)", fontSize: "0.78rem", display: "flex", alignItems: "center", gap: 6 }}>
                    <i className={`fas ${m.ic}`} style={{ color: "rgba(255,255,255,.75)" }} />{m.t}
                  </span>
                ))}
              </div>
            </div>
        ...
        <div className="row g-3 mb-4">
          {[
            { lbl: "Current SGPA", val: sgpa, sub: "Semester IV · 2025–26", grad: `linear-gradient(135deg,${C.navy},${C.blue2})`, icon: "fa-graduation-cap" },
            { lbl: "Overall Percentage", val: "87.4%", sub: "Distinction Category", grad: `linear-gradient(135deg,${C.blue},${C.blue3})`, icon: "fa-percent" },
            { lbl: "Subjects Cleared", val: "7 / 7", sub: "No backlogs pending", grad: `linear-gradient(135deg,${C.red},${C.red2})`, icon: "fa-book-open" },
            { lbl: "Class Rank", val: "#12", sub: "Out of 120 students", grad: `linear-gradient(135deg,#7f1d1d,${C.red2})`, icon: "fa-medal" },
          ].map((c, i) => (
            <div key={i} className="col-6 col-xl-3">
              <div style={{ background: c.grad, borderRadius: 14, padding: "20px 18px", color: "#fff", position: "relative", overflow: "hidden", boxShadow: "0 4px 16px rgba(12,36,97,.18)", height: "100%" }}>
                <div style={{ position: "absolute", right: -10, top: -10, fontSize: 52, opacity: 0.1 }}><i className={`fas ${c.icon}`} /></div>
        ...
        <Panel style={{ marginBottom: 24 }}>
          <PanelHeader icon="fa-table" title="Semester IV — Subject-wise Result" right="B.Tech · CS · Sem IV" />
        ...
        <div className="row g-3 mb-4">
          ...
          <div className="col-12 col-lg-5">
            <Panel style={{ height: "100%" }}>
              <PanelHeader icon="fa-chart-pie" title="Performance Analytics" right="Sem IV · 2025–26" />
              <div style={{ padding: 20 }}>
                {/* Donut + KPIs */}
                ...
                {/* Subject score bars */}
                <div style={{ fontWeight: 700, fontSize: "0.82rem", color: C.text, marginBottom: 10 }}>
                  <i className="fas fa-chart-bar me-2" style={{ color: C.blue2 }} />Subject Scores
                </div>
                    ...
                    return (
                      <div key={i} style={{ display: "grid", gridTemplateColumns: "100px 1fr 34px", alignItems: "center", gap: 8 }}>
                        ...
                        <div style={{ background: C.border, borderRadius: 20, height: 7, overflow: "hidden" }}>
                          <div style={{ width: barsOn ? `${tot}%` : "0%", height: "100%", borderRadius: 20, background: BAR_COLORS[i], transition: "width 0.8s ease" }} />
                        </div>
                        <span style={{ fontSize: "0.7rem", fontWeight: 800, textAlign: "right", color: BAR_COLORS[i] }}>{tot}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="col-12 col-lg-7">
            <Panel style={{ height: "100%" }}>
              <PanelHeader icon="fa-layer-group" title="Semester-wise Marks" right="All Semesters · B.Tech CS" />
              <div style={{ padding: 20 }}>
                {/* Sem selector */}
        ...
        <div className="row g-3 mb-4">
          <div className="col-12 col-md-6">
            <Panel style={{ height: "100%" }}>
              <PanelHeader icon="fa-clipboard-list" title="Recent Exams" right="View All" />
          ...
          <div className="col-12 col-md-6">
            <Panel style={{ height: "100%" }}>
              <PanelHeader icon="fa-bullhorn" title="Notices" />
              <div style={{ padding: "8px 20px 16px" }}>
                {NOTICES.map((n, i) => (
        ...
        {/* ── FOOTER ── */}
        <div style={{ textAlign: "center", color: C.muted, fontSize: "0.82rem", padding: "12px 0 4px", borderTop: `1px solid ${C.border}` }}>
          <i className="fas fa-shield-alt me-2" style={{ color: C.blue2 }} />
