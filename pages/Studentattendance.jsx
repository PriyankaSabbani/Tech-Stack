...
/* ─── HELPERS ─── */
const getBarColor = (p) => p >= 90 ? "linear-gradient(180deg,#1a56c4,#0d3b8c)" : p >= 75 ? "linear-gradient(180deg,#f59e0b,#d97706)" : "linear-gradient(180deg,#ef4444,#b91c1c)";
const getColor = (p) => p >= 90 ? "#0d3b8c" : p >= 75 ? "#d97706" : "#dc2626";
const getGradient = (p) => p >= 90 ? "linear-gradient(90deg,#1a56c4,#0d3b8c)" : p >= 75 ? "linear-gradient(90deg,#f59e0b,#d97706)" : "linear-gradient(90deg,#ef4444,#b91c1c)";
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        <div>
          <div style={{ color: "#06225a", fontWeight: 800, fontSize: 16 }}>🗓 {MONTHS[month]} {year}</div>
      ...
      <div className="row g-3">
        {[
          { label: "Present Days", val: summary.present, color: "#1d4ed8", bg: "#eff6ff", icon: "✅" },
          { label: "Absent Days", val: summary.absent, color: "#dc2626", bg: "#fff1f2", icon: "❌" },
        ].map((s) => (
...
export default function AttendancePage() {
  ...
  const { student } = user;
  const rollNo = `21CS0${String(student.id).padStart(2, '0')}`;

  const attendanceData = db.attendanceRecords.find(r => r.studentId === student.id);
  const summaryStats = attendanceData?.summaryStats || [];
  const monthlyData = attendanceData?.monthlyData || [];
  const attendanceMap = attendanceData?.attendanceMap || {};
