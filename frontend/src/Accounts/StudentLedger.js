...
import { getLedgerStudents, getStudentTransactions } from '../data/db';
...
export default function StudentLedger() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await getLedgerStudents();
      setStudents(data);
    };
    fetchData();
  }, []);
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      if (selectedStudent) {
        const txns = await getStudentTransactions(selectedStudent.id);
  ...
  let running = 0;
  const txnsWithBalance = transactions.map(t => { running = running + t.debit - t.credit; return { ...t, balance: running }; });
