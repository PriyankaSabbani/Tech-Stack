...
import { getFeeStructureData, addFeeStructure, updateFeeStructure, deleteFeeStructure } from '../data/db';
...
export default function FeeStructure() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await getFeeStructureData();
      setStructures(data);
    };
    fetchData();
  }, []);
  ...
  const handleSave = async () => {
    if (editId) {
      const updated = await updateFeeStructure(editId, { ...form, tuition: +form.tuition, library: +form.library, lab: +form.lab, exam: +form.exam, total: total(form) });
      setStructures(updated);
    } else {
      const updated = await addFeeStructure({ ...form, tuition: +form.tuition, library: +form.library, lab: +form.lab, exam: +form.exam, total: total(form) });
      setStructures(updated);
    }
  ...
  const handleDelete = async (id) => { const updated = await deleteFeeStructure(id); setStructures(updated); };
