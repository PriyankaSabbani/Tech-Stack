...
import { getFeeCollectionData, addFeeRecord } from '../data/db';
...
export default function FeeCollection() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await getFeeCollectionData();
      setFees(data);
    };
    fetchData();
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    const newFee = { ...form, total, paid, pending, status };
    const updatedFees = await addFeeRecord(newFee);
    setFees(updatedFees);
    setShowForm(false);
