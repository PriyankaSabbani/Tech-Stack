...
import { getRecentPayments, getMonthlyData, getDashboardStats } from '../data/db';
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', marginBottom: '28px' }}>
        {card('💰', 'Total Fees Collected', `₹${(stats.totalFeesCollected || 0).toLocaleString('en-IN')}`, '↑ 12% vs last month', '#1d4ed8', '#3b82f6')}
        {card('⏳', 'Pending Fees', `₹${(stats.pendingFees || 0).toLocaleString('en-IN')}`, `${stats.pendingInvoices || 0} students pending`, '#dc2626', '#f87171')}
        {card('💸', 'Total Expenses', `₹${(stats.totalExpenses || 0).toLocaleString('en-IN')}`, 'This academic year', '#d97706', '#f59e0b')}
        {card('📈', 'Net Balance', `₹${(stats.netBalance || 0).toLocaleString('en-IN')}`, 'After all deductions', '#059669', '#34d399')}
        {card('👔', 'Staff Salary Paid', `₹${(stats.staffSalaryPaid || 0).toLocaleString('en-IN')}`, 'Feb 2025 Payroll', '#7c3aed', '#a78bfa')}
        {card('🎓', 'Scholarships Given', `₹${(stats.scholarshipsGiven || 0).toLocaleString('en-IN')}`, `${stats.activeScholarships || 0} students benefited`, '#0891b2', '#22d3ee')}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginTop: '20px' }}>
        {[
          { label: 'Total Students', value: stats.totalStudents?.toLocaleString('en-IN') || '0', icon: '👨‍🎓', color: '#eff6ff', text: '#1d4ed8' },
          { label: 'Fees Collected Today', value: `₹${(stats.feesCollectedToday || 0).toLocaleString('en-IN')}`, icon: '📅', color: '#f0fdf4', text: '#059669' },
          { label: 'Pending Invoices', value: stats.pendingInvoices || '0', icon: '📌', color: '#fef2f2', text: '#dc2626' },
          { label: 'Active Scholarships', value: stats.activeScholarships || '0', icon: '🏅', color: '#faf5ff', text: '#7c3aed' },
        ].map((s, i) => (
